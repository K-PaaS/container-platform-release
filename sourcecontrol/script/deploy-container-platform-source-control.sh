...
# 2. Push the Container Platform Source Control Image & Helm chart in Harbor
for IMAGE in ${IMAGE_NAME[@]}
do
  if [[ "$PROVIDER_TYPE" == *standalone* ]] && [[ "$IMAGE" == *broker* ]]; then
      continue
  fi
...
# 3. Deploy the Container Platform Source Control
...
do
  ...
  sudo helm install -f ../values/$IMAGE_NAME-values.yaml $IMAGE_NAME $REPOSITORY_PROJECT_NAME/$IMAGE_NAME --namespace $NAMESPACE
done
